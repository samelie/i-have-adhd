...
```bash
codex plugin marketplace add samelie/i-have-adhd --ref main
...
A bundled `SessionStart` hook loads the full ruleset at the start of every session after you opt in:

```bash
touch ~/.codex/.i-have-adhd-always
```

Restart Codex, run `/hooks`, then review and trust the i-have-adhd hook. Codex skips new or changed plugin hooks until they are trusted.

If you use a custom Codex home directory, create the flag there instead:

```bash
touch "$CODEX_HOME/.i-have-adhd-always"
```
